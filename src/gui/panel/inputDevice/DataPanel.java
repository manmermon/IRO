...
{    
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5730442398914728652L;
	/**
	 * 
	 */
	 ...
	 * 
	 */
	public DataPanel( int xLen ) 
	{
		super();
		
		super.setBorder( BorderFactory.createEtchedBorder() );
		
		this.xLen = ( xLen < 10 ) ? 9 : xLen - 1;
	...
	private void drawAux( double[] values, Color lineColor, int currentIndex, Double[] currentSamples )
	{
		...
		for( int i = 0; i < currentSamples.length; i++ )
		{
			...
			ys[ i ] = (int)( this.height * ( 1 - ( currentSamples[ i ] - this.minY ) / ySpan ) );
		}
		...
		for( int i = currentSamples.length; i < xs.length; i++ )
		{
			...
			ys[ i ] = (int)( this.height *( 1 - (values[ i - currentSamples.length ] - this.minY ) / ySpan ) );
		}
		
		this.drawPolyLine( xs, ys, lineColor );				
	}
	
	private void adjustYAxis( double[] values )
	{
		for( double v : values )
		{
			this.maxY = ( v + 1 > this.maxY ) ? ( v + 1 ) : this.maxY;
			this.minY = ( ( v - 1 ) < this.minY ) ? ( v - 1 ) : this.minY;
		}
	}
