...
public class PlayerStatusIcon 
{
	...
	public static ImageIcon getPhysicalEffort( int level, int side, Color borderColor, Color fillColor )
	{
		int l = level;
		if( l < 1 )
		{
		...
		int thicknessBorder = 3;
		Image imgBase = BasicPainter2D.rectangle(side, side, thicknessBorder, borderColor, fillColor);
		...
		switch ( l )
		{
			case 1:
			{
				img = getSAMValence(  11, side, borderColor, fillColor );
				break;
			}
			case 2:
			{
				img = getSAMValence(  7, side, borderColor, fillColor );
				break;
			}
			case 3:
			{
				img = getSAMValence(  5, side, borderColor, fillColor );
				break;
			}
			case 4:
			{
				img = getSAMValence(  4, side, borderColor, fillColor );
				break;
			}
			case 5:
			{
				img = getSAMValence(  2, side, borderColor, fillColor );
				
				break;
			}
			case 6:
			{
				img = getBasicEmotion( 5, side, Color.BLACK, fillColor, "", null );
				break;
			}
			case 7:
			{				
				...
				imgBase = BasicPainter2D.composeImage(imgBase, side - side / 3 - leftEye.getWidth(null) / 4, side / 6, leftEye);

				imgBase = BasicPainter2D.composeImage(imgBase, side / 6 + rigthEye.getWidth(null) / 4, side / 6, rigthEye);
			
				imgBase = BasicPainter2D.composeImage( imgBase, side/2 - mouth.getWidth( null ) / 2 , imgBase.getHeight( null ) - imgBase.getHeight( null ) / 3  , mouth );
				
				img = new ImageIcon( imgBase );
			...
			case 8:
			{
				...
				int sweatW = (int)( rigthEye.getWidth( null ) * 0.75D) ;

				int x = thicknessBorder / 2;
				int y = thicknessBorder / 2;

				int w = sweatW - 2 * thicknessBorder;
				if (w < 1)
				{
					w = sweatW;
				}

				int sweatH = sweatW - thicknessBorder;
				if( sweatH <= 0 )
				{
					sweatH = 1;
				}
				
				Image sweatTip = BasicPainter2D.arc(0, 0, sweatW, sweatH, 0, -180, thicknessBorder, borderColor, fillColor, null);

				Image sweatBody = BasicPainter2D.createEmptyCanva(sweatTip.getWidth(null), sweatTip.getHeight(null), fillColor);

				BasicPainter2D.line(x, y, x, sweatBody.getHeight(null), thicknessBorder, borderColor, sweatBody);
				BasicPainter2D.line(sweatBody.getWidth(null) - x - 1, y, sweatBody.getWidth(null) - x - 1, sweatBody.getHeight(null), thicknessBorder, borderColor, sweatBody);
					
				Image sweat = BasicPainter2D.createEmptyCanva(sweatTip.getWidth(null), sweatTip.getHeight(null) / 2 + sweatBody.getHeight(null), fillColor);

				sweat = BasicPainter2D.composeImage(sweat, 0, 0, sweatBody);
				sweat = BasicPainter2D.composeImage(sweat, 0, sweatBody.getHeight(null) / 2, sweatTip);
				...
				imgBase = BasicPainter2D.composeImage(imgBase, side - (int)(sweat.getWidth(null)*1.25D), side / 12, sweat);
			...
			case 9:
			{
				img = getBasicEmotion( 4, side, Color.BLACK, fillColor, "", null );
				
				imgBase = img.getImage();
				
				int sweatW = (int)( eye.getWidth( null ) * 0.75D) ;
			...
			{
				Image rigthEye = BasicPainter2D.createEmptyCanva(eye.getWidth(null), eye.getHeight(null), fillColor);
				BasicPainter2D.line(0, 0, eye.getWidth(null), eye.getHeight(null), thicknessBorder, borderColor, rigthEye);
				BasicPainter2D.line(0, eye.getHeight(null), eye.getWidth(null), 0, thicknessBorder, borderColor, rigthEye);

				Image leftEye = BasicPainter2D.createEmptyCanva(eye.getWidth(null), eye.getHeight(null), fillColor);
				BasicPainter2D.line(0, 0, eye.getWidth(null), eye.getHeight(null), thicknessBorder, borderColor, leftEye);
				BasicPainter2D.line(0, eye.getHeight(null), eye.getWidth(null), 0, thicknessBorder, borderColor, leftEye);
				...
				imgBase = BasicPainter2D.composeImage(imgBase, (imgBase.getWidth(null) - mouthTongue.getWidth(null)) / 2, 
						imgBase.getHeight(null) - (int)(1.05D * mouthTongue.getHeight(null)), 
						mouthTongue);
